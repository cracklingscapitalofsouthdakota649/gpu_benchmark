...
echo Running Kubernetes Pipelines for GPU Benchmarks ...
...
set REPORT_IMAGE_TAG=luckyjoy/gpu-benchmark-report
...
echo Executing command: kubectl delete job gpu-benchmark-test-run --ignore-not-found=true
kubectl delete job gpu-benchmark-test-run --ignore-not-found=true
...
:: -----------------------------------------------------------
echo.
echo [STEP 5/10] Applying new Kubernetes Job definition (gpu-benchmark-job.yaml)...
echo Executing command: kubectl apply -f gpu-benchmark-job.yaml
...
:: -----------------------------------------------------------
echo.
echo [STEP 6/10] Checking status of gpu-benchmark Pods...
echo Executing command: kubectl get pods -l app=gpu-benchmark
kubectl get pods -l app=gpu-benchmark
...
echo Executing command: docker tag luckyjoy/gpu-benchmark-local:latest luckyjoy/gpu-benchmark-local:latest
docker tag luckyjoy/gpu-benchmark-local:latest luckyjoy/gpu-benchmark-local:latest
if %ERRORLEVEL% neq 0 (
...
echo Executing command: docker push luckyjoy/gpu-benchmark-local:latest
docker push luckyjoy/gpu-benchmark-local:latest
if %ERRORLEVEL% neq 0 (
...
FOR /F "tokens=*" %%p IN ('kubectl get pods -l app=gpu-benchmark --sort-by=.metadata.creationTimestamp -o jsonpath="{.items[0].metadata.name}" 2^>NUL') DO kubectl logs -f %%p
...
echo ====================================================================
echo.
echo BDD Test Image Location: https://hub.docker.com/r/luckyjoy/gpu-benchmark-local/tags
...
echo Image 'luckyjoy/gpu-benchmark-local:latest' is intentionally kept locally for performance on next run.
